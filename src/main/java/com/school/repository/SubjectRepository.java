...
public interface SubjectRepository extends JpaRepository<Subject, Long> {
    ...
    @Query(value = "select distinct subject from Subject subject left join fetch subject.gradeLevels",
    ...
    @Query("select distinct subject from Subject subject left join fetch subject.gradeLevels")
    List<Subject> findAllWithEagerRelationships();

    @Query("select subject from Subject subject left join fetch subject.gradeLevels where subject.id =:id")
